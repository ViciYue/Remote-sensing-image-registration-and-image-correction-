function [corrected_img, tform, stats] = sift_correction(data_folder)
% SIFT特征匹配与图像纠正
    ...
    %% 2. SIFT特征匹配与图像纠正
    % 特征检测与匹配
    points_distorted = detectSIFTFeatures(distorted_gray);
    points_reference = detectSIFTFeatures(reference_gray);
    ...
    index_pairs = matchFeatures(features_distorted, features_reference, 'MaxRatio', 0.6, 'Unique', true);
    ...
    [tform, inlier_idx] = estimateGeometricTransform2D(...
        matched_points_distorted.Location, matched_points_reference.Location, ...
        'projective', 'MaxDistance', 10);
    ...
    fprintf('SIFT图像纠正完成！找到%d个内点匹配\n', length(inlier_idx));
    fprintf('平均误差: %.2f 像素, 匹配成功率: %.1f%%\n', mean_error, inlier_ratio*100);

    %% 4. 显示校正结果图像
    show_sift_correction_results(distorted_img, reference_img, corrected_img, stats);
end

function show_sift_correction_results(distorted_img, reference_img, corrected_img, stats)
    % 显示SIFT校正结果
    
    % 创建主结果显示窗口
    figure('Name', '王紫航-SIFT图像校正结果');
    
    % 子图1: 原始扭曲图像
    subplot(2, 4, 1);
    imshow(distorted_img);
    title('原始扭曲图像', 'FontSize', 12, 'FontWeight', 'bold');
    ...
    title('SIFT校正后图像', 'FontSize', 12, 'FontWeight', 'bold');
    ...
    text(0.1, 0.95, info_text, 'FontSize', 10, 'VerticalAlignment', 'top', ...
